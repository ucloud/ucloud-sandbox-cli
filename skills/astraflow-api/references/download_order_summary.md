...
生成订单汇总 Excel 文件（包含"已完成订单"和"欠费订单"两个 sheet），并返回 US3 预签名下载链接。
...
| 参数 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| Action | string | 是 | 固定为 `DownloadOrderSummary` |
...
说明：本接口为同步返回下载链接的模式，响应中未出现 TaskId、TaskStatus 等异步任务相关字段；下载链接携带在 `Data.DownloadURL` 中，生成的 Excel 文件内包含"已完成订单"与"欠费订单"两个 sheet（文档未进一步说明各 sheet 内的具体列结构）。
...
https://astraflow.ucloud.cn/reference/modelverse/download_order_summary
