...
const authErrorBox = (keyName: string) => {
  ...
  return boxen.default(
    `You must be logged in to use this command. Run ${asBold('ucloud-sandbox-cli auth login')}.
...
export function ensureUserConfig(): UserConfig {
  const userConfig = getUserConfig()
  if (!userConfig) {
    console.error('No user config found, run `ucloud-sandbox-cli auth login` to log in first.')
    process.exit(1)
  }
  return userConfig
}
