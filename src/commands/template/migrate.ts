...
import { SandboxConfig, configName, getConfigPath, loadConfig } from '../../config'
...
async function migrateToLanguage(
  ...
  try {
    ...
    console.warn(
      `   ${asPrimary(
        `docker build -f ${defaultDockerfileName} --platform linux/amd64 -t your-image-tag .`
      )}`
    )
    console.warn(
...
export const migrateCommand = new commander.Command('migrate')
  .description(
    `migrate ${asLocal(defaultDockerfileName)} and ${asLocal(
      configName
    )} to new Template SDK format`
  )
  .option(
    '-d, --dockerfile <file>',
    `specify path to Dockerfile. Defaults to ${asLocal(defaultDockerfileName)}`
  )
